...
- style 속성은 만약 background-color는
  backgroundColor처럼 - 기호 대신 캐멀 케이스(camelCase)로
  사용해야 한다.
...
- 사실 스타일을 조작할 때는 style로 접근하는 방식보다 미리 클래스에
  스타일을 지정하고 classList로 해당 클래스를 추가, 삭제로 조작하는 것을 추천!
...
- 브라우저에서 사용자가 하는 모든 행위(마우스 클릭, 키보드 입력 등)를 이벤트(Even t)라 한다.
...
- 하나의 이벤트에 대해 여러 개의 이벤트 핸드러를 등록할 수 있다.
...
- 메타문자를 이용해서 정규표현시으로 검색조건을 삼을 "특정패턴"을 제시가능
...
- . : 임이의 문자 한 개를 의미하고, 문자 내용은 무엇이든 상관없다.
